...
create policy if not exists "scan feedback insert own" on public.scan_feedback
  for insert with check (auth.uid() = user_id or user_id is null);

create policy if not exists "scan feedback read own" on public.scan_feedback
  for select using (auth.uid() = user_id);

create policy if not exists "atm reports insert own or anonymous" on public.atm_fee_reports
  for insert with check (auth.uid() = user_id or user_id is null);

create policy if not exists "atm reports public read" on public.atm_fee_reports

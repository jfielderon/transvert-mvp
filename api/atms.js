...
function confidenceFor(operator, network, fee) {
...
async function geocodeManualLocation(query, apiKey) {
  ...
  if (!response.ok || payload.status !== 'OK') {
    return { center: undefined, warning: payload.error_message || payload.status || 'Geocoding failed' };
  }

  const location = payload.results?.[0]?.geometry?.location;
  if (typeof location?.lat !== 'number' || typeof location?.lng !== 'number') {
    return { center: undefined, warning: 'Geocoding returned no coordinates' };
  }
  return { center: { latitude: location.lat, longitude: location.lng } };
}
...
async function fetchGoogleAtms(latitude, longitude, apiKey, osmAtms) {
...
function osmOnlyAtms(latitude, longitude, osmAtms) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const google = apiKey ? await fetchGoogleAtms(center.latitude, center.longitude, apiKey, osmAtms) : { atms: [], warning: 'GOOGLE_MAPS_API_KEY is not available to the server function.' };
    if (google.warning) warnings.push(`Google Places: ${google.warning}`);

    const atms = google.atms.length > 0 ? google.atms : osmOnlyAtms(center.latitude, center.longitude, osmAtms);
    const provider = google.atms.length > 0 ? 'google-places+osm' : (osmAtms.length > 0 ? 'osm-overpass' : 'atm-intelligence');
